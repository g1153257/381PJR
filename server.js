...
var mongourl = 'mongodb://g1153257:kazZ09wW@ds141474.mlab.com:41474/g1153257';
...
var SECRETKEY = 'KEYKEYKEYKEYKEYKEYKEYKEY';
	...
	if (!req.session.authenticated) {
		...
		MongoClient.connect(mongourl, function(err, db) {
			...
			find1Restaurant(db,criteria,function(restaurant) {
				...
				res.render('showDetail', {r: restaurant});
				res.end();
			});
		});
	}
...
app.post('/newRest', function(req, res) {
	...
	MongoClient.connect(mongourl, function(err, db) {
		...
		db.collection('restaurant').insert(r,
			function(err,result) {
				console.log(result);
				assert.equal(err,null);
...
app.post('/api/create', function(req, res) {
	...
	MongoClient.connect(mongourl, function(err, db) {
		assert.equal(err,null);
		db.collection('restaurant').insert(r, function(err,result) {
				assert.equal(err,null);
				if(err) {
					res.json(result);
				}else{	
...
app.post('/edit', function(req, res) {
	...
	MongoClient.connect(mongourl, function(err, db) {
		assert.equal(err,null);
		console.log('Connected to MongoDB\n');
		db.collection('restaurant').update(criteria,item, function(err,result) {
			assert.equal(err,null);
...
app.get('/delete', function(req, res, next) {
	if (!req.session.authenticated) {
		...
		MongoClient.connect(mongourl, function(err, db) {
			if (req.session.username == req.query.createby) {
				db.collection('restaurant').deleteOne({"_id": ObjectID(req.query.id)}, function(err, result) {
      					assert.equal(null, err);
